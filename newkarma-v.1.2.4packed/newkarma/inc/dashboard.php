...
function gmr_render_dashboard_widget() {
	...
	if ( false !== $data_array && ! empty( $data_array ) && is_array( $data_array ) ) {
		...
		if ( ! empty( $imagebanner ) && isset( $imagebanner ) && ! empty( $imagebannerurl ) && isset( $imagebannerurl ) ) {
			...
			echo '<a href="' . esc_url( $imagebannerurl ) . '?utm_medium=dashboard&utm_source=newkarma" rel="nofollow" target="_blank"><img src="' . esc_url( $imagebanner ) . '" style="display:block;width:100%;" loading="lazy" /></a>';
			echo '</div>';
		}
		...
		if ( is_array( $themeterbaru ) ) {
			...
			foreach ( $themeterbaru as $value ) {
				if ( ! empty( $value['url'] ) && isset( $value['url'] ) && ! empty( $value['title'] ) && isset( $value['title'] ) ) {
					echo '<li><a href="' . esc_url( $value['url'] ) . '?utm_medium=dashboard&utm_source=newkarma" rel="nofollow" target="_blank">' . esc_attr( $value['title'] ) . '</a></li>';
				}
			}
			echo '</ul></div>';
		}
	...
	echo '<p class="community-events-footer" style="margin: 0 -12px -12px !important;background-color: #efefef;">';
		echo '<a href="https://member.kentooz.com/login?utm_medium=dashboard&utm_source=newkarma" target="_blank" rel="nofollow">Login Memberarea <span class="screen-reader-text">(opens in a new tab)</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a>';
		echo ' | ';
