...
if ( ! function_exists( 'gmr_theme_wrapper_start' ) ) :
	...
	function gmr_theme_wrapper_start() {
		...
		if ( 'fullwidth' === $class ) :
			$classes = wp_filter_nohtml_kses( 'col-md-main' );
		else :
			$classes = wp_filter_nohtml_kses( 'col-md-content newkarma-woocommerce' );
		endif;
		?>

		<div id="primary" class="content-area <?php echo $classes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>">
...
if ( ! function_exists( 'gmr_wp_nav_menu_items' ) ) :
	...
	function gmr_wp_nav_menu_items( $items, $args, $ajax = false ) {
		...
		if ( ( isset( $ajax ) && $ajax ) || ( property_exists( $args, 'theme_location' ) && 'topnav' === $args->theme_location && 0 === $mod ) ) {
			// WooCommerce.
			if ( class_exists( 'WooCommerce' ) ) {
				...
				$items         .= '<li class="' . esc_attr( $css_class ) . '">';
					$items     .= '<a class="cart-contents" href="' . esc_url( wc_get_cart_url() ) . '">';
						$items .= '<span class="icon_cart_alt"></span><sup>' . wp_kses_data( WC()->cart->get_cart_contents_count() ) . '</sup>';
					$items     .= '</a>';
				$items         .= '</li>';
				...
				$items         .= '<li class="' . esc_attr( $css_class ) . '">';
					$items     .= '<a class="cart-contents" href="' . esc_url( edd_get_checkout_uri() ) . '">';
						$items .= '<span class="icon_cart_alt"></span><sup>' . wp_kses_data( edd_get_cart_quantity() ) . '</sup>';
					$items     .= '</a>';
				$items         .= '</li>';
			}
		}
...
if ( ! function_exists( 'gmr_woocommerce_add_to_cart_fragments' ) ) :
	 ...
	 * @since 1.0.0
	 *
	 * @param string $fragments fragment.
	 * @return string
	 */
