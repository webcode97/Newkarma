...
if ( ! function_exists( 'customizer_library_get_font_choices' ) ) :
	...
	function customizer_library_get_all_fonts() {
		$heading1       = array( 1 => array( 'label' => sprintf( '--- %s ---', __( 'Standard Fonts', 'newkarma' ) ) ) );
		$standard_fonts = customizer_library_get_standard_fonts();
		$heading2       = array( 2 => array( 'label' => sprintf( '--- %s ---', __( 'Google Fonts', 'newkarma' ) ) ) );
...
if ( ! function_exists( 'customizer_library_get_google_font_uri' ) ) :
	 ...
	 *
	 * @since  1.0.0.
	 * @param array $fonts List google fonts.
	...
	function customizer_library_get_google_font_uri( $fonts ) {
		...
		foreach ( $fonts as $font ) {
			...
			if ( array_key_exists( $font, $allowed_fonts ) ) {
				...
				$family[] = rawurlencode( $font . ':' . join( ',', customizer_library_choose_google_font_variants( $font, $allowed_fonts[ $font ]['variants'] ) ) );
			}
		}
		...
		if ( ! empty( $subsets ) ) {
			$request .= rawurlencode( '&subset=' . join( ',', $subsets ) );
		}
...
if ( ! function_exists( 'customizer_library_get_google_font_subsets' ) ) :
	...
	function customizer_library_get_google_font_subsets() {
		return array(
			'all'          => __( 'All', 'newkarma' ),
			'cyrillic'     => __( 'Cyrillic', 'newkarma' ),
			'cyrillic-ext' => __( 'Cyrillic Extended', 'newkarma' ),
			'devanagari'   => __( 'Devanagari', 'newkarma' ),
			'greek'        => __( 'Greek', 'newkarma' ),
			'greek-ext'    => __( 'Greek Extended', 'newkarma' ),
			'khmer'        => __( 'Khmer', 'newkarma' ),
			'latin'        => __( 'Latin', 'newkarma' ),
			'latin-ext'    => __( 'Latin Extended', 'newkarma' ),
			'vietnamese'   => __( 'Vietnamese', 'newkarma' ),
		);
	}
endif;
...
if ( ! function_exists( 'customizer_library_choose_google_font_variants' ) ) :
	...
	function customizer_library_choose_google_font_variants( $font, $variants = array() ) {
		...
		if ( in_array( '500', $variants, true ) ) {
			$chosen_variants[] = '500';
		}

		// Only add "600" if it exists.
		if ( in_array( '600', $variants, true ) ) {
			$chosen_variants[] = '600';
		}
...
if ( ! function_exists( 'customizer_library_get_standard_fonts' ) ) :
	...
	function customizer_library_get_standard_fonts() {
		return array(
			'serif'      => array(
				'label' => _x( 'Serif', 'font style', 'newkarma' ),
				'stack' => 'Georgia,Times,"Times New Roman",serif',
			),
			'sans-serif' => array(
				'label' => _x( 'Sans Serif', 'font style', 'newkarma' ),
				'stack' => '"Helvetica Neue",sans-serif',
			),
			'monospace'  => array(
				'label' => _x( 'Monospaced', 'font style', 'newkarma' ),
...
if ( ! function_exists( 'customizer_library_get_google_fonts' ) ) :
	...
	function customizer_library_get_google_fonts() {
		return apply_filters(
			'customizer_library_get_google_fonts',
			array(
				'ABeeZee' => array(
					'label'    => 'ABeeZee',
					'variants' => array(
